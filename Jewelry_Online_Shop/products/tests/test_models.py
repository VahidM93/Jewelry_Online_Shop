...
class TestCategory(TestCase):
    ...
    def test_category_creation(self):
        self.assertEqual(self.category.name, 'Gold')
        self.assertEqual(self.category.slug, 'Gold')
        self.assertEqual(self.category.is_sub, False)
        self.assertEqual(self.category.sub_category, None)

    def test_category_str(self):
        self.assertEqual(str(self.category), 'Gold')
        self.assertNotEqual(str(self.category), 'phone')

    def test_category_get_absolute_url(self):
        self.assertEqual(self.category.get_absolute_url(), '/category/Gold/')
...
class TestProperty(TestCase):
    ...
    def test_property_creation(self):
        self.assertEqual(self.property1.key, '101')
        self.assertEqual(self.property1.priority, 1)

        self.assertEqual(self.property2.key, '201')
...
class TestProduct(TestCase):
    def setUp(self):
        ...
        self.product = Product.objects.create(
            name='Ring with diomond',
            slug='Ring with diomond',
            description='A great Gold with jewelry',
            price_no_discount=100000,
            price=900000,
            stock=12,
        )
        ...
        self.product2 = Product.objects.create(
            name='Silver ring',
            slug='Silver ring',
            description='A great Silver with low price',
            price_no_discount=10000,
            price=9000,
            stock=10,
        )
    ...
    def test_product_creation(self):
        self.assertEqual(self.product.name, 'Ring with green jewelry')
        self.assertEqual(self.product.slug, 'Ring with green jewelry')
        self.assertEqual(self.product.description, 'A great Gold with high price')
        self.assertEqual(self.product.price_no_discount, 100000)
    ...
    def test_product_str(self):
        self.assertEqual(str(self.product), 'Ring with green jewelry')
        self.assertNotEqual(str(self.product), 'Ring with red jewelry')

    def test_product_image_tage(self):
        self.assertEqual(self.product.image_tag(), '<img src="/media/" width="250" height="250" />')
